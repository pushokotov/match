...
class TinderClient:
    ...
    def _request(self, method: str, path: str, **kwargs: Any) -> requests.Response:
        ...
        if not response.ok:
            raise TinderAPIError(f"Tinder API returned {response.status_code}: {response.text[:500]}")
        return response
    ...
    def authenticate_with_phone_code(self, phone: str, code: str) -> str:
        ...
        response = self._request("POST", f"/v3/auth/login?locale={self.locale}", data=payload)
        match = re.search(r"(\\x12\\$)(.*)(\"\\x18)", response.text)
        if not match:
            raise TinderAPIError("Tinder authentication response did not contain an auth token")
        token = match.group(2)
        self.set_token(token)
        return token
    ...
    def get_profile(self) -> Profile:
        path = "/v2/profile?locale={}&include=account%2Cboost%2Ccontact_cards%2Cemail_settings%2Cinstagram%2Clikes%2Cnotifications%2Cplus_control%2Cproducts%2Cpurchase%2Creadreceipts%2Cswipenote%2Cspotify%2Csuper_likes%2Ctinder_u%2Ctravel%2Ctutorials%2Cuser".format(self.locale)
    ...
    def set_location(self, latitude: float, longitude: float) -> None:
        ...
        self._request("POST", f"/v2/meta?locale={self.locale}", json=payload)

    def get_recommendations(self) -> list[Recommendation]:
        data = self._request("GET", f"/v2/recs/core?locale={self.locale}").json().get("data", {})
        ...
        for item in results:
            ...
            recommendations.append(Recommendation(
                user=__import__("app.tinder.models", fromlist=["TinderUser"]).TinderUser(
                    id=user_id,
                    name=user.get("name", ""),
